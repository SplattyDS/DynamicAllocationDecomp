...
struct SaveData : FileSaveData, MinigameSaveData
{
	...
	static bool SaveCurrentFile();                                 // also saves minigames data, returns whether the save succeeded
	static bool SaveFile(u32 fileID, FileSaveData* saveData); // calls SaveDataToCart(saveData, 0x44,  fileID)
	static bool SaveMinigames(MinigameSaveData* saveData);         // calls SaveDataToCart(saveData, 0x2e4, 3)
